...
///
/// For any `String` arguments, the `%BIN_PATH%` and `%PORT%` variables are available
...
pub struct EnvArgs {
    ...
    #[arg(long, default_value_t = String::from("%BIN_PATH%/db/%PORT%.db"))]
    pub db_path: String,
    ...
    #[arg(long, default_value_t = String::from("%BIN_PATH%/files/%PORT%/"))]
...
fn replace_string_vars(to_replace: &str, env_args: &EnvArgs) -> String {
    ...
    to_replace = to_replace.replace("%BIN_PATH%", &bin_path);
...
impl ServerArgs {
    pub fn generate_big_file_path(&self, ts: &prost_types::Timestamp) -> String {
        self.env_args.big_file_directory.clone() + &ts.to_string()
    }
}
